        ...
        if (learningPoints >= MIN_LEARNING_POINTS) {
            ...
            if (abs(det) > 0.0001f) {
                ...
                linearCoeff = ((sumX4*sumXY - sumX3*sumX2Y + 
                              ...
                              sumY*sumX2*sumX2 - sumY*sumX3*sumX) / det);
...
float AdvancedBlockageDetector::calculateResistance(float pressureDiff, float flowRate) const {
    return (flowRate > FLOW_THRESHOLD) ? (pressureDiff / flowRate) : 0.0f;
}
...
bool AdvancedBlockageDetector::loadCalibration() {
    ...
    if (calibratedFlag == 0xCAFEBABE) {  // Magic number to verify calibration
    ...
    float inletPressure, float outletPressure, float flowRate) 
{
    ...
    const float normalizedResistance = baselineResistance > 0 ? 
        (currentResistance / baselineResistance) : 1.0f;
...
bool AdvancedBlockageDetector::validateReadings(float pressureDiff, float flowRate) {
    ...
    if(abs(pressureDiff - lastPressure) > 60.0f && flowRate < 5.0f) {  // Increased for 600 kPa range
        return false;
    }
